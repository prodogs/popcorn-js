...
optional:true},onEnd:{elem:"input",type:"function",label:"onEnd"}}})})(Popcorn);document.addEventListener("click",function(j){j=j.target;if(j.nodeName==="A"||j.parentNode&&j.parentNode.nodeName==="A")Popcorn.instances.forEach(function(d){d.options.pauseOnLinkClicked&&d.pause()})},false);(function(j){var d=/(?:http:\/\/www\.|http:\/\/|www\.|\.|^)(youtu|vimeo|soundcloud|baseplayer)/,f={},b={vimeo:false,youtube:false,soundcloud:false,module:false};Object.defineProperty(f,void 0,{get:function(){return b[void 0]},set:function(a){b[void 0]=a}});j.plugin("mediaspawner",{manifest:{about:{name:"Popcorn Media Spawner Plugin",version:"0.1",author:"Matthew Schranz, @mjschranz",website:"mschranz.wordpress.com"},options:{source:{elem:"input",type:"text",label:"Media Source","default":"http://www.youtube.com/watch?v=CXDstfD9eJ0"},
...
{position:n,pov:{heading:k.heading=k.heading||0,pitch:k.pitch=k.pitch||0,zoom:k.zoom}}));var y=function(p,v){var E=google.maps.geometry.spherical.computeHeading;setTimeout(function(){var x=u.media.currentTime;if(typeof k.tween==="object"){for(var t=0,F=p.length;t<F;t++){var w=p[t];if(x>=w.interval*(t+1)/1E3&&(x<=w.interval*(t+2)/1E3||x>=w.interval*F/1E3)){B.setPosition(new google.maps.LatLng(w.position.lat,w.position.lng));B.setPov({heading:w.pov.heading||E(w,p[t+1])||0,zoom:w.pov.zoom||0,pitch:w.pov.pitch||
0})}}y(p,p[0].interval)}else{t=0;for(F=p.length;t<F;t++){w=k.interval;if(x>=w*(t+1)/1E3&&(x<=w*(t+2)/1E3||x>=w*F/1E3)){z.setPov({heading:E(p[t],p[t+1])||0,zoom:k.zoom,pitch:k.pitch||0});z.setPosition(C[t])}}y(C,k.interval)}},v)};if(k.location&&typeof k.tween==="string"){var z=s,C=[],r=new google.maps.DirectionsService,D=new google.maps.DirectionsRenderer(z);r.route({origin:k.location,destination:k.tween,travelMode:google.maps.TravelMode.DRIVING},function(p,v){if(v==google.maps.DirectionsStatus.OK){D.setDirections(p);
for(var E=p.routes[0].overview_path,x=0,t=E.length;x<t;x++)C.push(new google.maps.LatLng(E[x].lat(),E[x].lng()));k.interval=k.interval||1E3;y(C,10)}})}else if(typeof k.tween==="object"){var B=s;r=0;for(var q=k.tween.length;r<q;r++){k.tween[r].interval=k.tween[r].interval||1E3;y(k.tween,10)}}}k.onmaploaded&&k.onmaploaded(k,l)}else setTimeout(function(){A()},13)};A()},end:function(){if(l)l.getDiv().style.display="none"},_teardown:function(i){var k=document.getElementById(i.target);k&&k.removeChild(h);
...
label:"Template"},data:{elem:"input",type:"text",label:"Data"},dynamic:{elem:"input",type:"checkbox",label:"Dynamic","default":true}}})})(Popcorn);(function(j){var d=0,f=function(b,a){var c=b.container=document.createElement("div"),e=c.style,g=b.media,h=function(){var l=b.position();e.fontSize="18px";e.width=g.offsetWidth+"px";e.top=l.top+g.offsetHeight-c.offsetHeight-40+"px";e.left=l.left+"px";setTimeout(h,10)};c.id=a||j.guid();e.position="absolute";e.color="white";e.textShadow="black 2px 2px 6px";e.fontWeight="bold";e.textAlign="center";h();b.media.parentNode.appendChild(c);return c};j.plugin("subtitle",{manifest:{about:{name:"Popcorn Subtitle Plugin",
...
"none"},_teardown:function(a){var c=a._target;c&&c.removeChild(a._container)}})})(Popcorn);
